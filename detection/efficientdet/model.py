...
class Regressor_KeyPoints(nn.Module):
    """
    modified by Zylo117
    """

    def __init__(self, in_channels, num_anchors, num_layers, pyramid_levels=5, onnx_export=False):
        super(Regressor_KeyPoints, self).__init__()
        self.num_layers = num_layers
        ...
        self.bn_list = nn.ModuleList(
            [nn.ModuleList([nn.BatchNorm2d(in_channels, momentum=0.01, eps=1e-3) for i in range(num_layers)]) for j in
             range(pyramid_levels)])
        self.header = SeparableConvBlock(in_channels, num_anchors * 8, norm=False, activation=False)
        self.swish = MemoryEfficientSwish() if not onnx_export else Swish()
    ...
    def forward(self, inputs):
        ...
        for feat, bn_list in zip(inputs, self.bn_list):
            ...
            feat = feat.permute(0, 2, 3, 1)
            feat = feat.contiguous().view(feat.shape[0], -1, 8)
