...
async def get_bboxes_and_diseases(file: UploadFile) -> JSONResponse:
    ...
    rois = rois * seg_map  # apply segmentation mask
    ...
    classification_rois = classification_rois.transpose(0, 3, 1, 2)
    classification_rois -= IMAGENET_MEAN
    classification_rois /= IMAGENET_STD
    mask = CLASSIFICATION_SESSION.batch_run(classification_rois, config.classification.batch_size).flatten() > 0
